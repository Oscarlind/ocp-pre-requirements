...
Assisted Installer needs to host an ISO file on an HTTP server. This ISO file is then used to boot the different nodes in order for them to join the OpenShift cluster. The HTTP server running on bastion will be used to host that ISO file.

TIP: When doing ZTP and if TLS is enabled for the HTTP server, it also needs a valid certificate that is trusted by the cluster

> NOTE: iPXE and USB can be used as alternatives to the HTTP server
